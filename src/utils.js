...
function logSecrets(secrets, source) {
  const masked = Object.fromEntries(Object.keys(secrets).map((k) => [k, '*'.repeat(5)]))
  info(chalk.yellow('secrets: ') + JSON.stringify(masked, null, 1) + '\n')
  log(chalk.green('√ ') + chalk.white('Secret loaded correctly from: ') + chalk.cyan(`< ${source} >`))
}
...
function setClientWithSSO(awsSecretsManagerConfig, counter = 1, throwError = true) {
  ...
  console.log(
    '\n' +
      converter.toOrdinal(counter) +
      ' attempt: Trying to retrieve secrets using profile: ' +
      chalk.cyan(JSON.stringify(awsSecretsManagerConfig.profile))
  )
