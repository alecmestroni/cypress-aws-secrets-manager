...
describe('getSecretFromAWS', () => {
    ...
    describe('when mandatory properties are missing', () => {
        it('AWS_SECRET_MANAGER_CONFIG: should return env and log a warning', async () => {
            const newEnv = { ...env };
            ...
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Continuing without secrets!'));
        });
    ...
    describe('when AWS_SECRETS_LOCAL_DIR is provided', () => {
        ...
        it('should console a warning and fetch secrets from AWS if file doesn\'t exists', async () => {
            ...
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Trying to fetch secrets from AWS Secrets Manager...'));
            expect(fs.existsSync).toHaveBeenCalledWith(fullFilePath);
